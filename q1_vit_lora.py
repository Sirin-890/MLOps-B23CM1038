...
WANDB_PROJECT = "Assignment-5-vit_lora"
HF_REPO = "vit_lora"
...
def build_base_model(num_classes=100, freeze_backbone=True):
    # Using timm's vit_small_patch16_224 pre-trained on ImageNet
...
def main():
    ...
    wandb.init(project=WANDB_PROJECT, name="Best_LoRA_Retrain", reinit=True)
    best_acc = run_experiment(best_lora_model, train_loader, val_loader, "Best_LoRA", epochs=10, device=device)
    wandb.finish()
    
    best_lora_model.save_pretrained("best_q1_model_new")
